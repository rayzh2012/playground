    ...
    for alias,ctx in zip(aliases,contexts):
        ...
        for s in p.get('hard_false_positive_substrings',[]):
            if s and s in text:
                matched=s
                break
        hard_fp.append(bool(matched))
        fp_rule.append(matched)
...
def main():
    ...
    research_totals=Counter()
    raw_total=0; research_total=0; hard_fp_total=0
    try:
        for p in occurrence_files:
            ...
            for batch in pf.iter_batches(batch_size=65536):
                ...
                raw_writer.write_table(raw)
                raw_total += raw.num_rows
                ...
                hard_fp_total += hard_fp_rows
                research_total += research.num_rows
                ...
                for alias in aliases:
                    ...
                    meta['raw_rows'] += sub.num_rows
                    sub_fp=sum(bool(x) for x in sub.column('hard_false_positive').to_pylist())
                    meta['hard_fp_rows'] += sub_fp
                    meta['research_rows'] += sub.num_rows-sub_fp
    ...
    with (out/'research_term_totals.csv').open('w',encoding='utf-8-sig',newline='') as f:
        ...
        for (corpus,alias),count in sorted(research_totals.items()):
            w.writerow([corpus,alias,count])
    ...
    with (out/'by_alias_index.csv').open('w',encoding='utf-8-sig',newline='') as f:
        ...
        for alias in sorted(alias_meta):
            ...
            w.writerow([alias,f'by_alias/{safe_alias_filename(alias)}',p.get('precision_class','UNCLASSIFIED'),
                        meta['raw_rows'],meta['research_rows'],meta['hard_fp_rows'],
                        '|'.join(sorted(meta['canonical'])),'|'.join(sorted(meta['entity_type']))])
